...
is still WIP since Hive SQL support for these tasks is not comprehensive.
The built-in Hive SQL functions `date_add`, `date_sub`, and `add_months` do
...
def build_ast(expr, context):
    ...
    builder = HiveQueryBuilder(expr, context=context)
...
def to_sql(expr, context=None):
    if context is None:
        context = HiveDialect.make_context()
    assert context is not None, 'context is None'
...
class HiveUDFNode(ops.ValueOp):
    def output_type(self):
        return rlz.shape_like(self.args, dtype=self.return_type)


class HiveUDAFNode(ops.Reduction):
    def output_type(self):
        return self.return_type.scalar_type()


class HiveSelectBuilder(comp.SelectBuilder):
    @property
    def _select_class(self):
        return HiveSelect


class HiveQueryBuilder(comp.QueryBuilder):
    select_builder = HiveSelectBuilder


class HiveContext(comp.QueryContext):
...
_hive_date_unit_names = {'Y': 'YEAR', 'Q': 'QUARTER'}

_hive_timestamp_unit_names = _hive_date_unit_names.copy()
_hive_timestamp_unit_names.update(
    {
        'M': 'MONTH',
        'W': 'WEEK',
        'D': 'DAY',
...
def _timestamp_truncate(translator, expr):
    ...
    arg_formatted = translator.translate(arg)
    try:
        unit = _hive_timestamp_unit_names[unit]
    except KeyError:
...
def _date_truncate(translator, expr):
    ...
    arg_formatted = translator.translate(arg)
    try:
        unit = _hive_date_unit_names[unit]
    except KeyError:
        raise com.UnsupportedOperationError(
...
def _timestamp_from_unix(translator, expr):
    ...
    return 'from_unixtime({})'.format(arg)
...
def _number_literal_format(translator, expr):
    ...
    if math.isfinite(value):
        if isinstance(expr.op().dtype, dt.Floating):
        ...
        if math.isnan(value):
            ...
            raise ValueError('Hive does not support Infinity')
...
_operation_registry.update(
    {
        ...
        ops.StrRight: fixed_arity('rtrim', 2),
...
class HiveExprTranslator(BaseExprTranslator):
    _registry = _operation_registry

    context_class = HiveContext


compiles = HiveExprTranslator.compiles
rewrites = HiveExprTranslator.rewrites


@compiles(ops.Arbitrary)
def hive_compiles_arbitrary(translator, expr):
    ...
    if how in (None, 'first'):
        ...
        raise com.UnsupportedOperationError(
            '{!r} value not supported for arbitrary in Hive SQL'.format(how)
        )


@compiles(ops.DayOfWeekName)
def hive_compiles_day_of_week_name(translator, expr):
    arg = expr.op().arg
    return 'date_format({}, {!r})'.format(translator.translate(arg), 'EEEE')


@compiles(ops.ExtractDayOfYear)
def hive_compiles_day_of_year(translator, expr):
    arg = expr.op().arg
    return 'date_format({}, {!r})'.format(translator.translate(arg), 'D')


class HiveSelect(BaseSelect):
    translator = HiveExprTranslator
